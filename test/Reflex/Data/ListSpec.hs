  ...
  let
    ...
    -- this includes the element itself!
      ...
      pushAddEvent n = do
        ...
        let foldfn :: (Int, Dynamic t Int) -> Int -> PushM t (Maybe Int)
...
push_enqueue_pop_dequeue_network ev = do
  let
    mdl = defaultDynamicListConfig
      { _dynamicListConfig_push    = fmapMaybe
        (\x -> if x `mod` 4 == 0 then Just x else Nothing)
        ev
      , _dynamicListConfig_enqueue = fmapMaybe
        (\x -> if x `mod` 4 == 1 then Just x else Nothing)
        ev
      , _dynamicListConfig_pop     = fmapMaybe
        (\x -> if x `mod` 4 == 2 then Just () else Nothing)
        ev
      , _dynamicListConfig_dequeue = fmapMaybe
        (\x -> if x `mod` 4 == 3 then Just () else Nothing)
        ev
      }
  dl <- holdDynamicList [] mdl
...
push_enqueue_pop_dequeue_test =
  TestLabel "push/enqueue/pop/dequeue" $ TestCase $ do
    let bs  = [0, 1, 0, 1, 0, 1, 0, 1, 2, 3, 3] :: [Int]
        run = runAppSimple push_enqueue_pop_dequeue_network bs
    v <- liftIO run
    let expected = Just [0, 0, 0, 1, 1]
    L.last v @?= [expected]
...
add_test = TestLabel "add" $ TestCase $ do
  ...
  v <- liftIO run
  let expected = fmap Just . L.tail . scanl (\acc x -> x : acc) [] $ bs
  join v @?= expected
